...
function standard_creep(creeper, creep_data)
    ...
    local refined_concrete_count = math.max(0, roboport.logistic_network.get_item_count("refined-concrete") - creep_data["minimum_item_count_setting"])
    local concrete_count = math.max(0, roboport.logistic_network.get_item_count("concrete") - creep_data["minimum_item_count_setting"])
    local brick_count = math.max(0, roboport.logistic_network.get_item_count("stone-brick") - creep_data["minimum_item_count_setting"])
    ...
    if creeper.upgrade then
        if #upgrade_target_types > 0 then
            local upgradable_tiles = surface.find_tiles_filtered { area = creep_data["area"], name = upgrade_target_types, limit = math.min(math.max(concrete_count, refined_concrete_count, 0), creep_data["usable_robots"]) }
...
function area_tile_creep(creeper, creep_data)
    ...
    local available_logistic_tile = math.max(0, roboport.logistic_network.get_item_count(logistic_area_tile) - minimum_item_count_setting)
    local available_construction_tile = math.max(0, roboport.logistic_network.get_item_count(construction_area_tile) - minimum_item_count_setting)
...
function space_creep(creeper, creep_data)
    ...
    local space_scaffold_count = math.max(0, roboport.logistic_network.get_item_count("se-space-platform-scaffold") - creep_data["minimum_item_count_setting"])
    local space_tile_count = math.max(0, roboport.logistic_network.get_item_count("se-space-platform-plating") - creep_data["minimum_item_count_setting"])
    ...
    if creeper.upgrade then
        if #upgrade_target_types > 0 then
            local upgradable_tiles = surface.find_tiles_filtered { area = creep_data["area"], name = upgrade_target_types, limit = math.min(math.max(space_tile_count, 0), creep_data["usable_robots"]) }
            for _, target_tile in pairs(upgradable_tiles) do
